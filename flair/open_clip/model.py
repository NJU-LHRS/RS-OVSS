...
class CLIP(nn.Module):
    ...
    def __init__(
        ...
        self.load_from_clip_pe = load_from_clip_pe       # TODO
        if not load_from_clip_pe == "vanilla":
        ...
        if not load_from_clip_pe == "vanilla":      # TODO
            if load_from_clip_pe == "false":
            ...
            elif load_from_clip_pe == "true":
                self.positional_embedding = nn.Parameter(torch.empty(77, text.width))
            else: raise 'Incorrect parameter for load_from_clip_pe.'
    ...
    def encode_text(self, text, normalize: bool = False):
        ...
        if self.load_from_clip_pe == "vanilla":      # TODO
    ...
    def forward(
        ...
        if 'batch' in kwargs:
            ...
            grid_size = self.visual.grid_size  # (H, W)
            features = {}
            # ======= Visual Encoder: Global =======
            global_image = batch["global_image"]        # TODO: 如果local用global的box，则在这，否则在下面的条件中
            # if any(loss in used_losses for loss in ["global_itc", "distill"]):
                # global_pooled, global_patches = self.visual(global_image, output_tokens=True)
                # features["global_image_patches"] = global_patches

                # global_pooled = self.visual(global_image)
            ...
            if any(loss in used_losses for loss in ["local_itc", "distill"]):
                # use_local_itc = "local_itc" in used_losses
                #
                # bboxes = batch["boxes"]
                ...
                #
                #     if has_local_categories: local_categories_list.append(local_categories[i][validity])
                #
                # if has_local_categories: features['local_categories'] = torch.cat(local_categories_list)
                #
                # # bboxes = batch["boxes"]
                # # local_images = batch["local_images"]
                # # local_texts = batch["local_texts"]
                #
                # # bboxes_list, local_imgs_list, local_texts_list = [], [], []
                # # for bbox, local_img, local_text in zip(bboxes, local_images, local_texts):
                # #     validity = bbox[:, -1] == 1
                # #     bboxes_list.append(bbox[validity, :4])
                # #     local_imgs_list.append(local_img[validity])
                # #     local_texts_list.append(local_text[validity])
                #
                # local_imgs = torch.cat(local_imgs_list)
                # local_pooled = self.visual(local_imgs, output_tokens=False)
                # local_pooled, local_patches = self.visual(local_imgs, output_tokens=True)
            ...
            # # ======= Distill Feature Extraction =======
            # # if "distill" in used_losses:
            # if "distill" in used_losses or "local_itc" in used_losses:        # FIXME: 为了测试pooled在local_itc的效果
            #     # ====== feat from global ======
            #     # global_patches = global_patches.view(len(global_image), grid_size[0], grid_size[1], -1).permute(0, 3, 1, 2)  # BHWC => BCHW   TODO: 改为在Transformer.PY中先进性proj而不是后proj
            #
            #     # h, w = global_patches.shape[-2:]  # 改为写在Transformer.py里
            #     # denormed_boxes = []
            ...
            #     # box_subset_global_features = roi_align(global_patches, denormed_boxes, (1, 1), 1.0, -1, True)[
            #     #     ..., 0, 0]
            #
            #     # roi_feats = roi_align(tokens, self._denormalize_boxes(normed_boxes, tokens),
            #     #                  (7, 7), 1.0, -1, True)  # [N, C, 7, 7]
            #     # return roi_feats.mean(dim=[2, 3])
            #
            #
            #     box_subset_global_features = self.visual.extract_roi_features(global_patches, bboxes_list)
            #     features["box_subset_global_features"] = box_subset_global_features
            #
            #
            #     # ====== feat from local ======
            #     # local_patches = local_patches.view(len(local_imgs), grid_size[0], grid_size[1], -1).permute(0, 3, 1, 2)  # BHWC => BCHW
            #
            #     # _bboxes_list = torch.tensor([[i, 0, 0, local_patches.shape[3], local_patches.shape[2]] for i in range(local_patches.shape[0])],
            #     #                     device=box_subset_global_features.device, dtype=box_subset_global_features.dtype)
            #     # averaged_local_features = roi_align(local_patches, _bboxes_list, (1, 1), 1.0, -1, True)[..., 0, 0]
            #
            #
            #     # local_patches = self.visual.extract_roi_features(local_imgs, _bboxes_list, last_attn_type, return_tokens=True)
            #     # averaged_local_features = roi_align(local_patches, _bboxes_list, (1, 1), 1.0, -1, True)[..., 0, 0]
            #
            #     # pooled = roi_align(local_patches, rois, (7, 7), 1.0, -1, True)
            #     # # averaged_local_features = pooled.mean(dim=[2, 3])
            #     # averaged_local_features = torch.amax(pooled, dim=[2, 3])
            #
            #
            #     _bboxes_list = [torch.tensor([[0, 0, 1, 1]], device=box_subset_global_features.device,
            #                                  dtype=box_subset_global_features.dtype) for _ in range(local_imgs.shape[0])]
            #     averaged_local_features = self.visual.extract_roi_features(local_patches, _bboxes_list)
            #
            #     features["local_image_patches_averaged"] = averaged_local_features
            ...
            if "local_itc" in used_losses:
                # local_texts = torch.cat(local_texts_list)
