...
class Transformer(nn.Module):
    ...
    def extract_feature_map(self, x, return_forward=False, last_attn_type='MaskCLIP', ignore_residual=True):
...
class VisionTransformer(nn.Module):
    ...
    def forward_v2(self, x, last_attn_type, return_pooled_tokens=False):
        ...
        # Patch tokens without last_attn
        ...
        if self.proj is not None:
            ...
            tokens = tokens @ self.proj                 # Patch tokens without last_attn
            pooled_tokens = pooled_tokens @ self.proj   # Pooled patch tokens without last_attn (GAP)
    ...
    # def extract_roi_features(self, x, normed_boxes, last_attn_type, return_tokens=False):
    #     x = self._embeds(x)
    #
    #     x = self.transformer.extract_feature_map(x, last_attn_type=last_attn_type)
    #
    ...
    #
    #     tokens = F.normalize(tokens, dim=-1)  # normalize along last dimension
    #     tokens = tokens.view(x.shape[0], self.grid_size[0], self.grid_size[1], -1).permute(0, 3, 1, 2)
    #
    #     if return_tokens:
    #         return tokens
    #
