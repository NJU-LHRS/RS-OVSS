...
# from tips.pytorch.TIPS_Demo import cls_token
...
class FLAIR(nn.Module):
    ...
    def encode_image(self, image, model_type, normalize: bool = False):
        global_image_token, local_image_tokens = self.visual(image, model_type=model_type)
    ...
    def get_seg_features(self, image, model_type='SegEarth'):
        """
        """
        global_image_token, local_image_tokens = self.encode_image(image, model_type=model_type)
        global_image_token, local_image_tokens = self.image_post(global_image_token), self.image_post(
            local_image_tokens) # (B, D), (B, L, D)
        batch_size = global_image_token.shape[0]
        ...
        # cls_logits = self.logit_scale.exp() * global_image_features @ text_features.t()
        # seg_logits = self.logit_scale.exp() * patch_features @ text_features.t()

        # cls_logits = global_image_features @ text_features.t()
        # seg_logits = patch_features @ text_features.t()
