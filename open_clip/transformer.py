...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, model_type: str = 'ClearCLIP', ignore_residual=True, output_cls_token=False, last_n_layers=1, return_last_attn_map=False):
        B, nc, w, h = x.shape
        ...
        if return_last_attn_map:
            blk = self.transformer.resblocks[-1]
            x_norm = blk.ln_1(x)
            attn_layer = blk.attn

            num_heads = attn_layer.num_heads
            num_tokens, bsz, embed_dim = x_norm.size()
            head_dim = embed_dim // num_heads

            q, k, v = F.linear(x_norm, attn_layer.in_proj_weight, attn_layer.in_proj_bias).chunk(3, dim=-1)
            q = q.contiguous().view(-1, bsz * num_heads, head_dim).transpose(0, 1)
            k = k.contiguous().view(-1, bsz * num_heads, head_dim).transpose(0, 1)
            v = v.contiguous().view(-1, bsz * num_heads, head_dim).transpose(0, 1)

            # attention
            attn_logits = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(head_dim)  # [B,H,N,N]
            attn_probs = attn_logits.softmax(dim=-1)

            # CLS -> patch
            cls_attn = attn_probs[:, 0, 1:]  # [B,H,N-1]
            return cls_attn
