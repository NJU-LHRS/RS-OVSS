...
class VisionTransformer(nn.Module):
  ...
  def forward_features(self, x, masks=None, model_type='SegEarth', ignore_residual=True):
    ...
    blk = self.blocks[-1]
    if ignore_residual:
      output = self.custom_attn(blk.attn, blk.norm1(x), model_type=model_type)
    else:
      x_out = x + self.custom_attn(blk.attn, blk.norm1(x), model_type=model_type)
      x_out = x_out + blk.mlp(blk.norm2(x_out))
      output = x_out
    x = output.permute(1, 0, 2)  # LND -> NLD
    # ==== end customized last layer ====
